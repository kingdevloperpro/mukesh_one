...
public class one_amd_DB_DB extends SQLiteOpenHelper {
    private static final String DATABASE_NAME = "myDatabase";
    private static final int DATABASE_VERSION = 1;

    public one_amd_DB_DB(Context context) {
    ...
    public void addLoan(one_lued_Foan_Ln_Dt SFLoanLnDt) {
    ...
    public List<one_lued_Foan_Ln_Dt> getAllLoan() {
        ArrayList arrayList = new ArrayList();
        Cursor rawQuery = getWritableDatabase().rawQuery("SELECT * FROM Loan ORDER BY id DESC", null);
        if (rawQuery.moveToFirst()) {
            do {
                one_lued_Foan_Ln_Dt SFLoanLnDt = new one_lued_Foan_Ln_Dt();
